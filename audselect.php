...
<body>
    <div>
       ...
       $sql = "SELECT serial_no, location, staff, category FROM stock";

        $result = $conn->query($sql);
        ?>

        <div id="auditselect_table"> 
          <form id="formName" action="generate_audit.php" method="post">
           <input type="submit" value="Generate Form" name="Continue">
  ...
  <th>Staff</th>
  <th>Category</th>
  <th><input type="checkbox" class="chk_boxes" label="check all"  />All</th>
  ...
  echo "<td> <input type='checkbox' class='chk_boxes1' name='checkser[]' value=".$row['serial_no']." /></td>";
  echo "</tr>";
  }
  echo "</table>";
  echo "<input type='submit' value='Submit Form' name='Continue'>";
  echo "</form>";
  echo "</div>";
